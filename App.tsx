...
import { GRID_SIZE, createInitialGrid, DIRECTION_ROTATION } from './constants';
import { Direction, GameState, BeeState } from './types';
...
const App: React.FC = () => {
    ...
    setGameState(prev => {
      ...
      if (cellContent === 'flower') {
        if (newCollected === prev.honeyTotal) {
          ...
          showMessage(`¡Falta miel! Tienes ${newCollected} de ${prev.honeyTotal}`);
        }
      }
  ...
  return (
    <div className="min-h-screen bg-yellow-50 flex items-center justify-center p-4 font-sans selection:bg-yellow-200">
      ...
      <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-12 gap-6 bg-white rounded-3xl shadow-2xl p-6 overflow-hidden border-[6px] border-yellow-300">
        
        {/* --- LEFT COLUMN: The Map (Grid) --- */}
        <div className="lg:col-span-8 flex flex-col items-center justify-center relative p-2">
          
          <h2 className="text-2xl font-black text-orange-500 mb-4 lg:hidden">La Aventura de Maya</h2>
          ...
          <div className="relative w-full max-w-xl aspect-square rounded-xl overflow-hidden shadow-2xl border-8 border-yellow-600 bg-slate-200">
            ...
            {isGeneratingMap && (
              <div className="absolute inset-0 z-40 bg-white/80 flex flex-col items-center justify-center">
                ...
                <p className="text-orange-600 font-bold text-xl animate-pulse">Pintando el aula...</p>
              </div>
            )}
        ...
        <div className="lg:col-span-4 flex flex-col items-center justify-between gap-6 bg-amber-50 rounded-2xl p-6 border-4 border-amber-100">
          ...
          <div className="text-center hidden lg:block">
            <h1 className="text-4xl font-black text-orange-500 tracking-tight drop-shadow-sm">Aventura<br/><span className="text-yellow-500">de Maya 🐝</span></h1>
            <p className="text-slate-500 font-bold mt-2">¡Lleva la miel a la flor!</p>
          </div>

          {/* Honey Jar Score */}
          <Jar collected={gameState.honeyCollected} total={gameState.honeyTotal} />

          {/* Controls */}
          <div className="bg-white p-6 rounded-[2rem] shadow-xl border-4 border-slate-100 w-full flex justify-center transform hover:scale-[1.02] transition-transform">
            <Controls 
          ...
          <div className="flex gap-4 w-full">
             <button 
              ...
              className="flex-1 bg-violet-100 hover:bg-violet-200 text-violet-700 py-3 rounded-2xl font-black transition-colors flex flex-col items-center justify-center gap-1 text-sm border-b-4 border-violet-300 active:border-b-0 active:translate-y-1"
              disabled={isGeneratingMap}
             >
               <span className="text-2xl">✨</span> 
               <span>Mapa Mágico</span>
             </button>
             
             <button 
              onClick={resetGame}
              className="flex-1 bg-blue-100 hover:bg-blue-200 text-blue-700 py-3 rounded-2xl font-black transition-colors flex flex-col items-center justify-center gap-1 text-sm border-b-4 border-blue-300 active:border-b-0 active:translate-y-1"
             >
               <span className="text-2xl">🔄</span> 
               <span>Reiniciar</span>
             </button>
      ...
      {gameState.status === 'won' && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl p-8 max-w-sm w-full text-center relative overflow-hidden transform transition-all scale-100 border-8 border-yellow-300">
            ...
            <h2 className="text-3xl font-black text-orange-500 mb-2">¡Lo conseguiste!</h2>
            <p className="text-slate-600 mb-8 text-lg font-medium leading-tight">Maya está muy contenta con toda su miel.</p>
